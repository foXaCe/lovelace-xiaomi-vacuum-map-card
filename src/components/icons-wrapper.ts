...
export class IconsWrapper extends RootlessLitElement {
    ...
    public static preprocessIcons(
        icons: IconActionConfig[] | undefined,
    ...
    private static expandMenus(
        ...
        return icons.flatMap((i) =>
            i.type === "menu" ? IconsWrapper.expandMenu(i as MenuIconActionConfig, internalVariables, hass) : [i]
        );
    }
    private static expandMenu(
        ...
        return values.map((v) => {
            return {
                ...icon,
                label:
                    icon.value_translation_keys?.[v] ??
                    IconsWrapper.getLabel(hass, entity, v, icon.current_value_attribute),
