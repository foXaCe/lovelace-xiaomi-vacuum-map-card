...
export default class PinchZoom extends HTMLElement {
    ...
    constructor() {
        ...
        const pointerTracker: PointerTracker = new PointerTracker(this, {
            start: (_pointer, event) => {
                if (
                    (<Element>event.target).classList.contains("draggable") &&
                ...
                if (
                    this.enablePan ||
                    pointerTracker.currentPointers.length == 1 ||
    ...
    attributeChangedCallback(name: string, _oldValue: string, newValue: string) {
        ...
        if (name === noDefaultPanAttr) {
            this.enablePan = !(newValue == "1" || newValue == "true");
        }
    ...
    set minScale(value: number) {
        if (value) this.setAttribute(minScaleAttr, String(value));
    }
    ...
    set maxScale(value: number) {
        if (value) this.setAttribute(maxScaleAttr, String(value));
    }
